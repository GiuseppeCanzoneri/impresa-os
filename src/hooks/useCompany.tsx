  ...
  async function refreshCompanies() {
    ...
    try {
      const { data, error: membershipsError } = await supabase
        ...
        .select('id, company_id, profile_id, role, is_active, created_at, company:companies(id, name, code, plan, status, created_at, updated_at)')
      ...
      const rows = (data ?? []) as unknown as Array<CompanyMember & { company?: Company }>;
      const nextMemberships = rows.map(({ company: _company, ...membership }) => membership);
      const nextCompanies = rows.map((row) => row.company).filter(Boolean) as Company[];
      ...
      const nextActiveId = storedStillValid ? storedId : firstCompanyId;

      setActiveCompanyIdState(nextActiveId);
      if (nextActiveId) localStorage.setItem(ACTIVE_COMPANY_STORAGE_KEY, nextActiveId);
    } catch (err) {
